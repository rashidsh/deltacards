...
(() => {
  ...
  const ACTION_SPECS = {
    ...
    dc_action_toggle_ability: actionSpec(
      ...
      [
        ...
        actionDropdown(
          'ABILITY',
          TOGGLEABLE_ABILITIES.map((ability) => [
  ...
  function normalizedPowers(raw, name) {
    ...
    raw.forEach((power, index) => {
      ...
      if (
        key === 'determination'
        || TOGGLEABLE_ABILITIES.includes(key)
      ) return;

      if (key === 'cantAttack') key = 'disarmed';
